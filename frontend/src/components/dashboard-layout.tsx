...
export default function DashboardLayout() {
  ...
  const [performanceSection, setPerformanceSection] = useState<"overview" | "conversations" | "sales" | "customer-experience" | "issues">("overview");
  ...
  const [importSource, setImportSource] = useState<"WhatsApp" | "Website" | "Email" | "SMS" | "Manual" | "Import" | "Google Contacts" | "Other">("Manual");
        ...
        <div className={`${selected === "Integrations" ? "max-w-[1600px]" : "max-w-7xl"} mx-auto h-full p-4 overflow-x-hidden`}>
          ...
          {selected === "Customers" && (() => {
            ...
            const handleSaveSegment = () => {
              ...
              if (segmentEditingId) {
                setSegments((current) => current.map((segment) => segment.id === segmentEditingId ? {
                  ...segment,
                  name,
                  description,
                  criteria,
            ...
            const filteredCustomerDirectoryItems = customerDirectoryItems
              .filter((item) => {
                const searchableText = [
                  item.name,
                  item.relationship,
              ...
              .sort((a, b) => {
                ...
                if (customerDirectorySort === "relationship") {
                  const order = { Contact: 0, Lead: 1, Customer: 2 };
                          ...
                          <article key={segment.id} className="rounded-[24px] border border-[#E5E7EB] bg-white p-5 shadow-[0_8px_24px_rgba(15,23,42,0.05)]">
                            ...
                            <div className="mt-4 rounded-[18px] border border-[#E5E7EB] bg-[#F9FAFB] p-3 text-sm text-[#475569]">
                              ...
                              <p className="mt-2 leading-6">
                                {segment.criteria || "No conditions added yet."}
                              </p>
                            </div>
                          ...
                          {filteredCustomerDirectoryItems.map((customer) => {
                            ...
                            return (
                              <tr
                                ...
                                className={`cursor-pointer border-t border-[#E5E7EB] align-top transition hover:bg-[#F8FAFC] focus:outline-none focus:ring-2 focus:ring-[#22C55E] focus:ring-offset-2 ${isSelected ? "bg-[#F3FDF7]" : ""}`}
                              >
                              ...
                              <td className="px-4 py-3">
                                <div className="flex flex-wrap gap-2">
                                  {customer.tags.length > 0 ? customer.tags.map((tag) => (
                                    <span key={`${customer.id}-${tag}`} className="rounded-full border border-[#E5E7EB] bg-[#F8FAFC] px-2 py-1 text-[11px] font-medium text-[#475569]">
                                      {tag}
                                    </span>
                                  )) : (
                                    <span className="text-[#94A3B8]">—</span>
                      ...
                      <div className="space-y-5 p-5">
                          ...
                          <div className="mt-3 space-y-4 text-sm text-[#475569]">
                            <div>
                              ...
                              <ul className="mt-2 space-y-1">
                                {selectedCustomer.phoneNumbers.length > 0 ? selectedCustomer.phoneNumbers.map((phone) => (
                                  <li key={`${selectedCustomer.id}-phone-${phone}`}>{phone}</li>
                            ...
                            <div>
                              ...
                              <ul className="mt-2 space-y-1">
                                {selectedCustomer.emailAddresses.length > 0 ? selectedCustomer.emailAddresses.map((email) => (
                                  <li key={`${selectedCustomer.id}-email-${email}`}>{email}</li>
                            ...
                            <div>
                              ...
                              <ul className="mt-2 space-y-1">
                                {selectedCustomer.otherContactMethods.length > 0 ? selectedCustomer.otherContactMethods.map((method) => (
                                  <li key={`${selectedCustomer.id}-method-${method}`}>{method}</li>
                        ...
                        <div className="rounded-[20px] border border-[#E5E7EB] bg-white p-4">
                          ...
                          <ul className="mt-3 space-y-2 text-sm text-[#475569]">
                            {selectedCustomer.recentInteractions.length > 0 ? selectedCustomer.recentInteractions.map((interaction) => (
                        ...
                        <div className="rounded-[20px] border border-[#E5E7EB] bg-white p-4">
                          ...
                          <div className="mt-3 flex flex-wrap gap-2">
                            {selectedCustomer.tags.length > 0 ? selectedCustomer.tags.map((tag) => (
                              <span key={`${selectedCustomer.id}-tag-${tag}`} className="rounded-full border border-[#E5E7EB] bg-[#F8FAFC] px-2 py-1 text-[11px] font-medium text-[#475569]">
                                {tag}
                              </span>
...
<div className="mt-4 flex flex-wrap items-center gap-3">
          ...
          {selected === "Performance" && (
            <div className={`space-y-6 ${CARD}`}>
              <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
                <div className="max-w-2xl">
                  ...
                  <h2 className="mt-2 text-2xl font-semibold text-[#111827]">
                    See how your AI employee is doing.
                  </h2>
                  <p className="mt-2 text-sm leading-6 text-[#6B7280]">
              ...
              <Tabs value={performanceSection} onValueChange={(value) => setPerformanceSection(value as typeof performanceSection)} className="w-full">
                      ...
                      <div className="mt-4 space-y-3">
                        {(() => {
                          const repeatedQuestions = Array.from(
                            new Set(
                              INBOX_CONVERSATIONS
                                .filter((conversation) => conversation.message.toLowerCase().includes("pricing") || conversation.message.toLowerCase().includes("price") || conversation.message.toLowerCase().includes("product") || conversation.message.toLowerCase().includes("installation"))
                                .map((conversation) => {
                                  const message = conversation.message.toLowerCase();
                                  if (message.includes("pricing") || message.includes("price")) return "Pricing questions";
